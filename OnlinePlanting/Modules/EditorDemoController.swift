...
class EditorDemoController: UIViewController {
    ...
    fileprivate(set) lazy var richTextView: Aztec.TextView = {
        ...
        textView.tintColor = UIColor.init(hexString: OPGreenColor)
        textView.textColor = UIColor.init(hexString: OPDarkGreenColor)
    ...
    fileprivate(set) lazy var titleTextField: UITextView = {        
        ...
        textField.tintColor = UIColor.init(hexString: OPGreenColor)
        textField.textColor = UIColor.init(hexString: OPDarkGreenColor)
    ...
    fileprivate(set) lazy var titlePlaceholderLabel: UILabel = {
        let placeholderText = NSLocalizedString("请输入标题", comment: "Label for the title of the post field. Should be the same as WP core.")
    ...
    fileprivate(set) lazy var contentPlaceholderLabel: UILabel = {
        let placeholderText = NSLocalizedString("请输入内容", comment: "Label for the content of the post field. Should be the same as WP core.")
        let contentTextField = UILabel()
        
        contentTextField.attributedText = NSAttributedString(string: placeholderText,
                                                      attributes: [NSForegroundColorAttributeName: UIColor.lightGray, NSFontAttributeName: UIFont.preferredFont(forTextStyle: UIFontTextStyle.body)])
    ...
    func updateContentPlaceholderVisibility() {
        self.contentPlaceholderLabel.isHidden = !richTextView.text.isEmpty
    }
    ...
    private func configureConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            separatorView.topAnchor.constraint(equalTo: titleTextField.bottomAnchor, constant: 4),
        ...
        NSLayoutConstraint.activate([
            ...
            richTextView.topAnchor.constraint(equalTo: view.topAnchor, constant: 4),
            richTextView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -Constants.margin)
            ])

        NSLayoutConstraint.activate([
            contentPlaceholderLabel.leadingAnchor.constraint(equalTo: titlePlaceholderLabel.leadingAnchor),
            contentPlaceholderLabel.topAnchor.constraint(equalTo: separatorView.bottomAnchor, constant: 8),
            contentPlaceholderLabel.heightAnchor.constraint(equalToConstant: richTextView.font!.lineHeight)
            ])
...
extension EditorDemoController : UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        case richTextView:
            updateFormatBar()
            updateContentPlaceholderVisibility()
        default:
            break
        }
    }
...
extension EditorDemoController : Aztec.FormatBarDelegate {
    ...
    func createToolbar() -> Aztec.FormatBar {
        ...
        toolbar.tintColor = UIColor.init(hexString: OPDarkGreenColor)
        toolbar.highlightedTintColor = UIColor.init(hexString: OPDarkGreenColor)
        toolbar.selectedTintColor = UIColor.init(hexString: OPGreenColor)
        toolbar.disabledTintColor = UIColor.init(hexString: OPDarkGreenColor)
        toolbar.dividerTintColor = UIColor.init(hexString: OPDarkGreenColor)
...
private extension EditorDemoController
{
    ...
    func insertImage(_ image: UIImage) {
        ...
        attachment.progressColor = UIColor.init(hexString: OPDarkGreenColor)
        let imageID = attachment.identifier

        OPDataService.sharedInstance.uploadImageToServer("cool", desc: "Good", uploadImage: image, imageID: imageID, handler: {(success, url, error) in
            if success {
                if let imageUrl = url {
                attachment.url = URL.init(string: imageUrl)
                }
            }
        }) { [weak self] (progress, imageId) in
            self?.updateProgressBar(progress,imageID: imageId)
        }
    }
    ...
    func updateProgressBar(_ progress: Progress?, imageID: String?) {
